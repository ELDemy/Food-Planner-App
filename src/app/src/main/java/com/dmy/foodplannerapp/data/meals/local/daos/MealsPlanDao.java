...
public interface MealsPlanDao {
    ...
    Flowable<List<MealPlanWithDetails>> getMealsByDate(Date date);

    @Transaction
    @Query("SELECT * FROM mealsPlan")
    Flowable<List<MealPlanWithDetails>> getMeals();

    @Transaction
    @Query("SELECT * FROM mealsPlan WHERE date = :date AND mealType = :mealType")
    Flowable<List<MealPlanWithDetails>> getMealsByDateAndType(Date date, String mealType);
    ...
    Flowable<List<Date>> getDatesWithMeals(Date startDate, Date endDate);

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    Completable insert(MealPlan mealPlan);

    @Delete
    Completable delete(MealPlan mealPlan);
